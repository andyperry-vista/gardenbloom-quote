...
export default function QuoteView() {
  ...
  const handleCreateJob = async () => {
    setCreatingJob(true);
    try {
      // Find the client_id from the quote's DB record
      const { data: dbQuote } = await supabase.from("quotes").select("client_id").eq("id", quote.id).single();
      if (!dbQuote?.client_id) throw new Error("No client linked");
      const result = await createJob({ quoteId: quote.id, clientId: dbQuote.client_id });
  ...
  const handleDelete = () => { deleteQuote(quote.id); navigate("/admin"); toast.success("Quote deleted"); };
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link to={`/admin/quotes/${id}/edit`}><Button variant="outline" size="icon"><Pencil className="w-4 h-4" /></Button></Link>
            <Select value={quote.status} onValueChange={(v) => updateQuote(quote.id, { status: v as Quote["status"] })}>
          ...
          <div className="bg-primary p-8 text-primary-foreground">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-sm opacity-80">{new Date(quote.createdAt).toLocaleDateString("en-AU")}</p>
              </div>
            </div>
          </div>
          <CardContent className="p-8 space-y-6">
            <div>
              <h3 className="text-sm font-medium text-muted-foreground mb-1">Prepared For</h3>
            ...
            <div>
              ...
              {quote.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="grid grid-cols-12 gap-2 py-3 border-b last:border-0 text-sm">
                    <div className="col-span-5"><span className="font-medium">{item.description}</span><span className="ml-2 text-xs text-muted-foreground capitalize">({item.type})</span></div>
                    <div className="col-span-2 text-right">{item.quantity}</div>
                    <div className="col-span-2 text-right">${clientPrice.toFixed(2)}</div>
            ...
            {quote.notes && (<><Separator /><div><h3 className="text-sm font-medium text-muted-foreground mb-1">Notes</h3><p className="text-sm">{quote.notes}</p></div></>)}
          </CardContent>
        </Card>

        <Card>
          <CardContent className="pt-6">
            <h3 className="text-sm font-medium text-muted-foreground mb-3">Your Margin Breakdown (internal)</h3>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div><p className="text-2xl font-bold">${quote.subtotal.toFixed(2)}</p><p className="text-xs text-muted-foreground">Wholesale Cost</p></div>
              <div><p className="text-2xl font-bold text-accent">${quote.markupTotal.toFixed(2)}</p><p className="text-xs text-muted-foreground">Your Markup</p></div>
              <div><p className="text-2xl font-bold text-primary">${quote.grandTotal.toFixed(2)}</p><p className="text-xs text-muted-foreground">Client Price</p></div>
            </div>
          </CardContent>
        </Card>
